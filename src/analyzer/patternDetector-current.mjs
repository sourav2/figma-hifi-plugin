import { nameHintScore } from '../utils/nodeHelpers.js';
import { PATTERN_TYPES } from './patterns.js';
import { logScan } from '../utils/logger.js';
import {
  DISPLAY_MIN_CONFIDENCE,
  getConfidenceLevel,
  shouldDisplayPattern,
} from './confidenceLevels.js';

/** @deprecated Use DISPLAY_MIN_CONFIDENCE — kept for imports. */
export const MIN_CONFIDENCE = DISPLAY_MIN_CONFIDENCE;

/**
 * Clamps and rounds confidence to 0–1 (two decimals).
 */
...
/**
 * Human-readable composition from extracted features.
 */
export function describeComposition(features) {
  const parts = [];
  ...
  if (features.textCount > 0) {
    parts.push(
      features.textCount === 1
        ? 'Text'
        : features.textCount + ' Text',
    );
  }
...
/**
 * Runs weighted indicators, negative penalties, and required signal gates.
 * Caps confidence to 0.29 if any required signal condition is unmet.
 */
function evaluatePattern(patternType, features, indicators, requiredSignals = [], negativeSignals = []) {
  ...
  // 1. Evaluate standard indicators
  ...
  // 2. Evaluate negative signals (penalties)
  for (let i = 0; i < negativeSignals.length; i++) {
    ...
    if (active) {
      ...
      explanation.push({
        ...
        passed: false, // In UI, fail highlights that penalty occurred
        weight: neg.penalty,
        type: 'penalty',
      });
  ...
  // Apply Requirement 2: Strong Name-Based Scoring boosts and penalties
  const nodeNameLower = (features.name || '').toLowerCase();
  
  if (/\b(button|btn|cta|submit|enquire)\b/i.test(nodeNameLower) || 
      nodeNameLower.indexOf('button') !== -1 || 
      nodeNameLower.indexOf('btn') !== -1 || 
      nodeNameLower.indexOf('cta') !== -1 ||
      nodeNameLower.indexOf('submit') !== -1 ||
      nodeNameLower.indexOf('enquire') !== -1) {
  ...
  if (/\b(input|field|textfield|form-field)\b/i.test(nodeNameLower) || 
      nodeNameLower.indexOf('input') !== -1 || 
      nodeNameLower.indexOf('textfield') !== -1 || 
      nodeNameLower.indexOf('field') !== -1 ||
      nodeNameLower.indexOf('form-field') !== -1) {
  ...
  if (/\b(header|navbar|navigation|menu|hamburger)\b/i.test(nodeNameLower) || 
      nodeNameLower.indexOf('header') !== -1 || 
      nodeNameLower.indexOf('navbar') !== -1 || 
      nodeNameLower.indexOf('navigation') !== -1 ||
      nodeNameLower.indexOf('menu') !== -1 ||
      nodeNameLower.indexOf('hamburger') !== -1) {
  ...
  if (/\b(card|course|product|item|tile)\b/i.test(nodeNameLower) || 
      nodeNameLower.indexOf('card') !== -1 || 
      nodeNameLower.indexOf('course') !== -1 || 
      nodeNameLower.indexOf('product') !== -1 ||
      nodeNameLower.indexOf('item') !== -1 ||
      nodeNameLower.indexOf('tile') !== -1) {
    if (patternType === 'Card') {
      score += 0.40;
  ...
  // 4. Apply required gates (cap below display threshold if missing required elements)
  if (!allRequiredMet) {
...
function scoreBannerWeighted(f) {
  ...
  const negative = [
    { label: 'Aspect ratio < 2.0', penalty: 0.5, test: (x) => x.aspectRatio < 2.0 },
...
function hasComponentSignals(features) {
  ...
  const structureSignals =
    features.childButtonsCount >= 1 ||
    features.childInputsCount >= 1 ||
    features.childCardsCount >= 1 ||
    features.childImagesCount >= 1 ||
    features.childLinksCount >= 1 ||
    features.childNavigationsCount >= 1 ||
    features.childMenusCount >= 1 ||
    features.childAvatarsCount >= 1 ||
    features.hasCtaButton ||
    features.hasHeading ||
    features.hasDescription ||
    features.hasImage ||
    features.hasImagePlaceholder;
...
function getStructuralRole(features) {
  ...
  const looksLikePageContainer =
    (features.candidateDepth <= 1 || features.childCount >= 2) &&
    (features.width >= 320 && features.height >= 480 || /\b(page|screen|main|home|app)\b/i.test(nameLower)) &&
    !semanticSectionSignals &&
    !componentSignals;
  ...
  const looksLikeLayoutContainer =
    !semanticSectionSignals &&
    !componentSignals &&
    (features.childCount >= 1 || features.isAutoLayout || features.isNestedGroup || /\b(wrapper|container|group|layout|frame|content|body)\b/i.test(nameLower));
...
function getDetectionLevel(features) {
  ...
  if (
    features.hasHeaderIntent ||
    features.hasFooterIntent ||
    features.hasHeroCarouselIntent ||
    hasSectionName ||
    (hasStructuralChildren && features.childCount >= 2 && (features.textCount >= 2 || features.childCount >= 3))
  ) {
    return 'section';
  }
...
/**
 * Scores all pattern types with weighted indicators, penalties, and required conditions.
 */
export function detectPatternWithDebug(features) {
  ...
  if (!eligibility.eligible) {
    const confidenceLevel = getConfidenceLevel(0);
    return {
  ...
  // Determine Intent Families
  const isFormFamily = (features.childInputsCount >= 2 && features.childButtonsCount >= 1) || features.childInputsCount >= 3;

  const isCardFamily = features.childCardsCount >= 1 || (
    (features.hasRectangle || features.cornerRadius > 0 || features.hasEffects || features.childCount >= 2) &&
    (features.hasHeading || features.maxFontSize >= 15 || (features.name && /title|heading|card/i.test(features.name))) &&
    (features.textCount >= 2 || (features.texts && features.texts.some(t => /hr|hour|week|month|day|duration/i.test(t.characters || '')))) &&
    (features.hasCtaButton || features.childButtonsCount >= 1 || (features.texts && features.texts.some(t => /enroll|apply|join|start|learn|enquire|view/i.test(t.characters || ''))))
  );

  const isCarouselFamily = (
    (features.name && /carousel|slider|slideshow/i.test(features.name)) ||
    (features.shapeCount >= 3 || (features.name && /dot|pagination|indicator/i.test(features.name))) ||
    (features.childCount >= 2 && (features.childCardsCount >= 2 || features.childImagesCount >= 2)) ||
    (features.name && /arrow|prev|next|chevron/i.test(features.name))
  );

  const isNavigationFamily = (
    (features.textSemantics && features.textSemantics.navKeywordCount >= 1) ||
    (features.childMenusCount >= 1 || features.hasAdjacentAvatarOrIcon || (features.name && /menu|hamburger|navbar|nav|header|icon/i.test(features.name))) ||
    (features.name && /breadcrumb/i.test(features.name)) ||
    (features.childCount >= 2 && (features.name && /tab|segment/i.test(features.name)))
  );

  const hasHeaderIntent =
    (features.isNearTop || features.y < 120) &&
    (features.childLinksCount >= 1 || features.childMenusCount >= 1 || features.childAvatarsCount >= 1 || (features.name && /header|navbar|nav/i.test(features.name)));

  const hasFooterIntent =
    (features.relativeY >= 0.6 || (features.name && /footer/i.test(features.name))) &&
    (features.childLinksCount >= 1 || features.textCount >= 2);

  const hasBodyContainerIntent =
    features.width >= 320 &&
    features.height >= 480 &&
    (features.childCount >= 2 || features.childNavigationsCount >= 1 || features.childCardsCount >= 1 || features.childLinksCount >= 1 || features.childInputsCount >= 1) &&
    !hasHeaderIntent &&
    !hasFooterIntent;

  const hasHeroCarouselIntent =
    (features.isCarouselFamily || (features.name && /hero|carousel|slider/i.test(features.name))) &&
    (features.childImagesCount >= 1 || features.childCardsCount >= 1) &&
    (features.textCount >= 2 || (features.name && /hero|banner/i.test(features.name)));

  const hasCourseCardIntent =
    (features.name && /course|learning|academy|class/i.test(features.name)) ||
    (features.texts && features.texts.some(t => /course|learning|academy|class|hr|hour|week|month|day/i.test(t.characters || '')));

  const hasFormSectionIntent =
    features.childInputsCount >= 2 && features.childButtonsCount >= 1;
  ...
  for (let j = 0; j < categoryResults.length; j++) {
    ...
    const rejectReasons =
      r.failedReasons.length > 0
        ? r.failedReasons
        : ['Score below winning category'];
  ...
  if (displayed && top) {
    ...
    const childSummaryString = childSummaryParts.length > 0 ? childSummaryParts.join(' · ') : 'None';
    ...
    if (SECTION_TYPES.has(top.type)) {
      ...
      // 4. Rejected list
      const topRejects = rejectedCategories
        .slice(0, 2)
        .map(cat => cat.type)
        .join(', ');
  ...
  const result = {
    ...
    topMatch: top
      ...
      : null,
    secondMatch: second
      ? { type: second.type, confidence: second.confidence }
      : null,
    ...
    allScores: allScores,
    rejectionReason: displayed
      ? null
      : buildRejectionSummary(composition, top, second, confidenceGap),
    textSemantics: features.textSemantics || null,
    placeholderCount: features.placeholderCount || 0,
    meaningfulCount: features.meaningfulCount || 0,
    placeholderTypes: features.placeholderTypes || [],
...
function buildRejectionSummary(composition, top, second, gap) {
  ...
  msg +=
    '. Not shown: top match below ' +
    Math.round(DISPLAY_MIN_CONFIDENCE * 100) +
    '% display threshold.';
...
/**
 * Classifies a node feature set into the best matching UI pattern.
 */
export function detectPattern(features) {
...
function applyCompetitionAdjustments(results, f) {
  ...
  // A. Intent Family Boosts and Penalties
  if (f.isFormFamily) {
    ...
    applyPenalty(resCardGrid, 0.60, 'Form Family active conflict');
    applyPenalty(resCardGridSection, 0.60, 'Form Family active conflict');
    applyPenalty(resCarousel, 0.60, 'Form Family active conflict');
    applyPenalty(resCarouselSection, 0.60, 'Form Family active conflict');
    applyPenalty(resImageGridSection, 0.60, 'Form Family active conflict');
    applyPenalty(resPlaceholder, 0.60, 'Form Family active conflict');
    applyPenalty(resLinkGroup, 0.60, 'Form Family active conflict');
  }

  if (f.hasFormSectionIntent) {
    applyBoost(resFormSection, 0.20, 'Form section intent');
    applyPenalty(resLinkGroup, 0.70, 'Form section intent');
    applyPenalty(findResult('Input Field'), 0.60, 'Form section intent');
  }
  ...
  if (f.isCardFamily) {
    ...
    applyBoost(resCardGrid, 0.30, 'Card Family active (Card Grid)');
    applyBoost(resCardGridSection, 0.30, 'Card Family active (Card Grid Section)');
    applyBoost(resCourseListingSection, 0.35, 'Card Family active (Course Listing Section)');
    
    applyPenalty(resPlaceholder, 0.50, 'Card Family active conflict');
    applyPenalty(resLinkGroup, 0.50, 'Card Family active conflict');
  }
  ...
  if (f.isCarouselFamily) {
    ...
    applyBoost(resHeroCarousel, 0.40, 'Carousel Family active (Hero Carousel)');
    
    applyPenalty(resCardGrid, 0.40, 'Carousel Family active conflict');
    applyPenalty(resCardGridSection, 0.40, 'Carousel Family active conflict');
    applyPenalty(resPlaceholder, 0.50, 'Carousel Family active conflict');
    applyPenalty(resLinkGroup, 0.50, 'Carousel Family active conflict');
  ...
  if (f.hasHeroCarouselIntent) {
    applyBoost(resHeroCarousel, 0.30, 'Hero carousel intent');
    applyPenalty(resHeader, 0.35, 'Hero carousel intent');
    applyPenalty(resHeaderSection, 0.25, 'Hero carousel intent');
    applyPenalty(resNav, 0.20, 'Hero carousel intent');
    applyPenalty(findResult('Description'), 0.40, 'Hero carousel intent');
  }

  if (f.isNavigationFamily) {
    applyBoost(resNav, 0.40, 'Navigation Family active');
    applyBoost(resHeader, 0.40, 'Navigation Family active');
    applyBoost(resHeaderSection, 0.40, 'Navigation Family active');
    ...
    applyBoost(resFilterGroup, 0.30, 'Navigation Family active (Filter Group)');
    applyBoost(resBreadcrumb, 0.30, 'Navigation Family active');
    
    applyPenalty(resCardGrid, 0.50, 'Navigation Family active conflict');
    applyPenalty(resCardGridSection, 0.50, 'Navigation Family active conflict');
    applyPenalty(resPlaceholder, 0.60, 'Navigation Family active conflict');
  }

  if (f.hasHeaderIntent) {
    applyBoost(resHeader, 0.20, 'Header intent');
    applyBoost(resHeaderSection, 0.20, 'Header intent');
    applyPenalty(resBodyContainer, 0.20, 'Header intent');
  }

  if (f.hasFooterIntent) {
    applyBoost(resFooterSection, 0.25, 'Footer intent');
    applyBoost(resFooterLinks, 0.20, 'Footer intent');
  ...
  if (f.hasBodyContainerIntent) {
    ...
    applyPenalty(resNav, 0.20, 'Body container intent');
  }

  if (f.hasCourseCardIntent) {
    applyBoost(resCourseCard, 0.20, 'Course card intent');
    applyPenalty(findResult('Modal'), 0.55, 'Course card intent');
  }

  // B. Parent Purpose Detection Priority
  ...
  if (hasMultipleChildren || hasStructuralChildren) {
    if (f.childInputsCount >= 1 || f.childButtonsCount >= 1) {
      applyPenalty(findResult('Image Block'), 0.90, 'Parent contains input/button components');
      applyPenalty(findResult('Link Group'), 0.90, 'Parent contains input/button components');
      applyPenalty(findResult('PlaceholderContent'), 0.90, 'Parent contains input/button components');
      applyPenalty(findResult('Button'), 0.90, 'Container layout takes priority over button');
      applyPenalty(findResult('Input Field'), 0.90, 'Container layout takes priority over input');
    }
    if (f.childCardsCount >= 1 || f.childImagesCount >= 1) {
      applyPenalty(findResult('Image Block'), 0.80, 'Parent contains card/image components');
      applyPenalty(findResult('Link Group'), 0.80, 'Parent contains card/image components');
      applyPenalty(findResult('PlaceholderContent'), 0.80, 'Parent contains card/image components');
    }
  }

  // C. Traditional structural adjustments & placeholders
  const hasStructuralLayout = (
    (resCarousel && resCarousel.confidence >= 0.3) ||
    (resCarouselSection && resCarouselSection.confidence >= 0.3) ||
    (resHeroCarousel && resHeroCarousel.confidence >= 0.3) ||
    (resCardGrid && resCardGrid.confidence >= 0.3) ||
    (resCardGridSection && resCardGridSection.confidence >= 0.3) ||
    (resCourseListingSection && resCourseListingSection.confidence >= 0.3) ||
    (resImageGridSection && resImageGridSection.confidence >= 0.3) ||
    (resFeatureListSection && resFeatureListSection.confidence >= 0.3)
  );

  if (hasStructuralLayout) {
    applyPenalty(resPlaceholder, 0.80, 'Structural layout active');
  ...
  // Top of screen boosts
  if (f.isNearTop || f.y < 120) {
    applyBoost(resNav, 0.40, 'Near top of screen');
    applyBoost(resHeader, 0.30, 'Near top of screen');
    applyBoost(resHeaderSection, 0.30, 'Near top of screen');
    applyBoost(resNavLinks, 0.25, 'Near top of screen');
  }

  // Footer / bottom boosts
  if (f.relativeY >= 0.6 || (f.name && /footer/i.test(f.name))) {
    applyBoost(resFooterLinks, 0.40, 'Near bottom/Footer context');
    applyBoost(resFooterSection, 0.40, 'Near bottom/Footer context');
    applyBoost(resLinkGroup, 0.30, 'Footer context');
    applyPenalty(resCarousel, 0.30, 'Footer/bottom context');
    applyPenalty(resCarouselSection, 0.30, 'Footer/bottom context');
    applyPenalty(resHeroCarousel, 0.30, 'Footer/bottom context');
    applyPenalty(resCardGrid, 0.20, 'Footer/bottom context');
    applyPenalty(resCardGridSection, 0.20, 'Footer/bottom context');
    applyPenalty(resCourseListingSection, 0.20, 'Footer/bottom context');
    applyPenalty(resImageGridSection, 0.20, 'Footer/bottom context');
  }

  // Link Group competition
  const linkGroupActive = (resLinkGroup && resLinkGroup.confidence >= 0.5) || 
                          (resFooterLinks && resFooterLinks.confidence >= 0.5) || 
                          (resNavLinks && resNavLinks.confidence >= 0.5);
  if (linkGroupActive) {
    applyPenalty(resCardGrid, 0.40, 'Link group active');
    applyPenalty(resCardGridSection, 0.40, 'Link group active');
    applyPenalty(resCourseListingSection, 0.40, 'Link group active');
    applyPenalty(resCarousel, 0.40, 'Link group active');
    applyPenalty(resCarouselSection, 0.40, 'Link group active');
    applyPenalty(resHeroCarousel, 0.40, 'Link group active');
    applyPenalty(resCard, 0.30, 'Link group active');
    applyPenalty(resCourseCard, 0.30, 'Link group active');
    applyPenalty(resContentCard, 0.30, 'Link group active');
    applyPenalty(resContentBlock, 0.20, 'Link group active');
    applyPenalty(resImageGridSection, 0.40, 'Link group active');
  }

  // No meaningful content penalty
  if (f.meaningfulCount === 0 && f.placeholderCount > 0) {
    applyPenalty(resCarousel, 0.50, 'No meaningful content');
    applyPenalty(resCarouselSection, 0.50, 'No meaningful content');
    applyPenalty(resHeroCarousel, 0.50, 'No meaningful content');
    applyPenalty(resCardGrid, 0.50, 'No meaningful content');
    applyPenalty(resCardGridSection, 0.50, 'No meaningful content');
    applyPenalty(resCourseListingSection, 0.50, 'No meaningful content');
    applyPenalty(resCard, 0.50, 'No meaningful content');
    applyPenalty(resCourseCard, 0.50, 'No meaningful content');
    applyPenalty(resContentCard, 0.50, 'No meaningful content');
    applyPenalty(resContentBlock, 0.40, 'No meaningful content');
    applyPenalty(resImageGridSection, 0.50, 'No meaningful content');
  }

  // Geometry-only checks
  if (f.textCount === 0) {
    applyPenalty(resNav, 0.80, 'No text content');
    applyPenalty(resLinkGroup, 0.80, 'No text content');
    applyPenalty(resFooterLinks, 0.80, 'No text content');
    applyPenalty(resFooterSection, 0.80, 'No text content');
    applyPenalty(resNavLinks, 0.80, 'No text content');
    applyPenalty(resBreadcrumb, 0.80, 'No text content');
    applyPenalty(resContentBlock, 0.80, 'No text content');
    applyPenalty(resCardGridSection, 0.80, 'No text content');
    applyPenalty(resCourseListingSection, 0.80, 'No text content');
    applyPenalty(resHeaderSection, 0.80, 'No text content');
    applyPenalty(resImageGridSection, 0.80, 'No text content');
    applyPenalty(resCarouselSection, 0.80, 'No text content');
    applyPenalty(resHeroCarousel, 0.80, 'No text content');
    applyPenalty(resFeatureListSection, 0.80, 'No text content');
  }

  // Navigation or Header Section active: Decrease Dropdown, Carousel, Card Grid
  const isNavActive = (resNav && resNav.confidence >= 0.5) || 
                      (resHeader && resHeader.confidence >= 0.5) ||
                      (resHeaderSection && resHeaderSection.confidence >= 0.5);
  if (isNavActive) {
    applyPenalty(resDropdown, 0.80, 'Navigation active');
    applyPenalty(resCarousel, 0.80, 'Navigation active');
    applyPenalty(resCarouselSection, 0.80, 'Navigation active');
    applyPenalty(resHeroCarousel, 0.80, 'Navigation active');
    applyPenalty(resCardGrid, 0.80, 'Navigation active');
    applyPenalty(resCardGridSection, 0.80, 'Navigation active');
    applyPenalty(resCourseListingSection, 0.80, 'Navigation active');
    applyPenalty(resImageGridSection, 0.80, 'Navigation active');
  }

  // Hero Section Competition: Exclude Textarea, Input, Dropdown, Cards, Carousels, Image Block, Link Group
  if (resHero && resHero.confidence >= 0.5) {
    applyPenalty(resTextarea, 0.80, 'Hero section active');
    applyPenalty(resInputField, 0.80, 'Hero section active');
    applyPenalty(resDropdown, 0.80, 'Hero section active');
    applyPenalty(resCardGrid, 0.80, 'Hero section active');
    applyPenalty(resCardGridSection, 0.80, 'Hero section active');
    applyPenalty(resCourseListingSection, 0.80, 'Hero section active');
    applyPenalty(resCarousel, 0.80, 'Hero section active');
    applyPenalty(resCarouselSection, 0.80, 'Hero section active');
    applyPenalty(resHeroCarousel, 0.80, 'Hero section active');
    applyPenalty(resCard, 0.80, 'Hero section active');
    applyPenalty(resCourseCard, 0.80, 'Hero section active');
    applyPenalty(resContentCard, 0.80, 'Hero section active');
    applyPenalty(resContentBlock, 0.80, 'Hero section active');
    applyPenalty(resCtaSection, 0.60, 'Hero section active');
    applyPenalty(resImageGridSection, 0.80, 'Hero section active');
    applyPenalty(findResult('Image Block'), 0.80, 'Hero section active');
    applyPenalty(findResult('Link Group'), 0.80, 'Hero section active');
    applyPenalty(findResult('PlaceholderContent'), 0.80, 'Hero section active');
  }

  // Form / Form Section Competition: Exclude Textarea, Input Field, Link Group, Image Block, and Content Block
  const isFormActive = (resForm && resForm.confidence >= 0.5) || (resFormSection && resFormSection.confidence >= 0.5);
  if (isFormActive) {
    applyPenalty(resInputField, 0.90, 'Form active');
    applyPenalty(resLinkGroup, 0.90, 'Form active');
    applyPenalty(findResult('Image Block'), 0.90, 'Form active');
    applyPenalty(resTextarea, 0.80, 'Form active');
    applyPenalty(resContentBlock, 0.80, 'Form active');
    applyPenalty(resCtaSection, 0.50, 'Form active');
    applyBoost(resForm, 0.15, 'Strong form components');
    applyBoost(resFormSection, 0.15, 'Strong form components');
  }

  // Carousel Section active: Exclude Image Block, PlaceholderContent, Link Group, and Card Grids
  ...
  if (isCarouselSectionActive) {
    applyPenalty(findResult('Image Block'), 0.80, 'Carousel section active');
    applyPenalty(findResult('PlaceholderContent'), 0.80, 'Carousel section active');
    applyPenalty(findResult('Link Group'), 0.80, 'Carousel section active');
    applyPenalty(resCardGrid, 0.50, 'Carousel section active');
    applyPenalty(resCardGridSection, 0.50, 'Carousel section active');
  }

  // Course Listing Section active: Exclude Card Grid, Link Group, Image Block
  const isCourseListingActive = (resCourseListingSection && resCourseListingSection.confidence >= 0.5);
  if (isCourseListingActive) {
    applyPenalty(resCardGrid, 0.50, 'Course Listing Section active');
    applyPenalty(resCardGridSection, 0.50, 'Course Listing Section active');
    applyPenalty(resLinkGroup, 0.80, 'Course Listing Section active');
    applyPenalty(findResult('Image Block'), 0.80, 'Course Listing Section active');
  }

  // Course Card active: Exclude generic Card, Content Card, Link Group, Image Block
  const isCourseCardActive = (resCourseCard && resCourseCard.confidence >= 0.5);
  if (isCourseCardActive) {
    applyPenalty(resCard, 0.50, 'Course Card active');
    applyPenalty(resContentCard, 0.50, 'Course Card active');
    applyPenalty(resLinkGroup, 0.80, 'Course Card active');
    applyPenalty(findResult('Image Block'), 0.80, 'Course Card active');
  }

  // Category Selector active: Exclude Link Group, Card Grid, Image Block, PlaceholderContent
  const isCategorySelectorActive = (resCategorySelector && resCategorySelector.confidence >= 0.5);
  if (isCategorySelectorActive) {
    applyPenalty(resLinkGroup, 0.80, 'Category Selector active');
    applyPenalty(resCardGrid, 0.80, 'Category Selector active');
    applyPenalty(findResult('Image Block'), 0.80, 'Category Selector active');
    applyPenalty(findResult('PlaceholderContent'), 0.80, 'Category Selector active');
  }

  // Content Block Competition
  if (resContentBlock && resContentBlock.confidence >= 0.5) {
    applyPenalty(resTextarea, 0.70, 'Content block active');
    applyPenalty(resCard, 0.50, 'Content block active');
    applyPenalty(resCourseCard, 0.50, 'Content block active');
    applyPenalty(resContentCard, 0.50, 'Content block active');
    applyPenalty(resBanner, 0.40, 'Content block active');
    applyPenalty(resHeader, 0.40, 'Content block active');
  }

  // CTA Section Competition
  if (resCtaSection && resCtaSection.confidence >= 0.5) {
    applyPenalty(resTextarea, 0.70, 'CTA section active');
    applyPenalty(resBanner, 0.40, 'CTA section active');
    applyPenalty(resHeader, 0.40, 'CTA section active');
  }

  // Navigation Icon Competition
  if (resNavigationIcon && resNavigationIcon.confidence >= 0.5) {
    applyPenalty(findResult('Checkbox'), 0.80, 'Navigation icon active');
    applyPenalty(findResult('Radio Group'), 0.80, 'Navigation icon active');
    applyPenalty(findResult('Input Field'), 0.80, 'Navigation icon active');
  }

  // Image Grid Section Competition
  if (resImageGridSection && resImageGridSection.confidence >= 0.5) {
  }

  // Carousel Pagination Dots Boost Pattern (Image Placeholder + Pagination Dots)
  const hasCarouselDotsPattern = (f.hasImage || f.hasImagePlaceholder || f.childImagesCount >= 1) && 
                                 (f.shapeCount >= 3 || (f.name && /dot|pagination|indicator/i.test(f.name)));
  if (hasCarouselDotsPattern) {
    applyBoost(resCarousel, 0.50, 'Image placeholder + pagination dots pattern detected');
    applyBoost(resCarouselSection, 0.50, 'Image placeholder + pagination dots pattern detected');
    applyBoost(resHeroCarousel, 0.50, 'Image placeholder + pagination dots pattern detected');
    applyPenalty(findResult('Image Block'), 0.80, 'Carousel pagination dots boost conflict');
    applyPenalty(findResult('PlaceholderContent'), 0.80, 'Carousel pagination dots boost conflict');
  }

  // Body Container Competition Adjustments
  const isBodyContainerActive = (resBodyContainer && resBodyContainer.confidence >= 0.5);
  if (isBodyContainerActive) {
    applyPenalty(resNavLinks, 0.80, 'Body Container active');
    applyPenalty(resLinkGroup, 0.80, 'Body Container active');
  }
}

// 38. Image Grid Section
